...
module.exports = function Mongoosastic(schema, options) {
  ...
  schema.methods.index = function(options, cb) {
    var doc = new this.constructor(this)
    ...
    doc.populate({path: populated.join(' ')}, function(err, doc) {
      if (bulk) {
        /**
         * To serialize in bulk it needs the _id
         */
        var serialModel = serialize(doc, mapping);
        serialModel._id = doc._id;
  ...
  function setUpMiddlewareHooks(schema) {
    ...
    function updateParent(filed) {
      return function(doc) {
        var model = options.model();

        var query = {};
        query[filed] = doc._id;
        options.reversequery && options.reversequery(query, doc)
    ...
    for (var field in schema.tree) {
      ...
      {
        if (schema.paths[field].instance.toLowerCase() !== 'objectid' &&
          schema.paths[field].instance.toLowerCase() !== 'array')
        {
          continue;
        }
        ...
        if (value[0] && value[0].es_schema) {
          _schema = value[0].es_schema;
        } else if (value.es_schema) {
          _schema = value.es_schema;
        }
        ...
        if (populated.indexOf(field) <= 0) {
          populated.push(field);
        }

        _schema.post('save', updateParent(field));
        _schema.post('remove', updateParent(field));
