  ...
  for (var field in tree) {
    ...
    if (type) {
      ...
      if (value[0]) {
        ...
        } else if (value[0].es_indexed && value[0].es_schema && value[0].es_schema.tree && value[0].es_schema.paths) {
          cleanTree[field] = getCleanTree(value[0].es_schema.tree, value[0].es_schema.paths, '');
      ...
      } else if (type === 'objectid' && value.es_indexed && value.es_schema && value.es_schema.tree && value.es_schema.paths) {
        cleanTree[field] = getCleanTree(value.es_schema.tree, value.es_schema.paths, '');
